...
### バックアップの保存場所
- 指定したパスにバックアップファイルを保存します。（初期値：`autosaver`）
...
## 保存ファイル形式
- 保存時のファイル名の形式を指定します。（初期値：`%PROJECTNAME%_%F_%R`）
...
## 更新履歴
### r3_gr_8
- 特定の状況で、バックアップファイルを開いている旨のメッセージが2回出てしまう問題を追加修正
### r3_gr_7
- プロジェクト名が特定の日本語文字列の場合、保存時にエラーが発生する問題を修正
- AviUtlで扱えないファイルパスになった場合、警告を表示する処理を追加
- 保存ファイル名形式の設定が保存されない問題を修正
### r3_gr_6
- コードを全体的に書き直し、ファイルサイズを5分の1に縮小
- 設定ファイルの文字コードをUTF-8→Shift-JISに変更
  - バックアップフォルダや名前に日本語を含んでいた場合、再設定を促すように指示されます。
- 特定の状況で、バックアップファイルを開いている旨のメッセージが2回出てしまう問題を修正
### r3_gr_5
- `.aup_backup`などという謎規格を取っていたのを止め、`.aup`で出力するように変更
- バックアップフォルダからプロジェクトファイルを開いた場合、警告を表示するように変更
- その他、軽微な変更
### r3_gr_4
